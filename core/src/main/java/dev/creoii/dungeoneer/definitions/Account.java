...
public record Account(
    ...
    public Account(long id, String username, String passwordHash, int characterSlots, LocalDateTime lastLoginDate) {
        this(id, username, passwordHash, characterSlots, createEmptyCharacters(characterSlots), -1L, null, lastLoginDate, "");
    }

    public Account copyWithFaction(long factionId, @Nullable LocalDateTime factionJoinDate) {
        return new Account(id, username, passwordHash, characterSlots, characters, factionId, factionJoinDate, lastLoginDate, settings);
    }
