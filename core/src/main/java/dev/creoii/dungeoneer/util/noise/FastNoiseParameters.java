package dev.creoii.dungeoneer.util.noise;
...
public record FastNoiseParameters(String id, long seed, float frequency, FastNoiseLite.NoiseType noiseType, FastNoiseLite.RotationType3D rotationType3D, Fractal fractal, Cellular cellular, DomainWarp domainWarp) implements Identifiable {
    public static final Codec<FastNoiseParameters> CODEC = RecordCodecBuilder.create(instance -> {
        return instance.group(
            ...
            Fractal.CODEC.fieldOf("fractal").orElse(Fractal.DEFAULT).forGetter(FastNoiseParameters::fractal),
            Cellular.CODEC.fieldOf("cellular").orElse(Cellular.DEFAULT).forGetter(FastNoiseParameters::cellular),
            DomainWarp.CODEC.fieldOf("domain_warp").orElse(DomainWarp.DEFAULT).forGetter(FastNoiseParameters::domainWarp)
