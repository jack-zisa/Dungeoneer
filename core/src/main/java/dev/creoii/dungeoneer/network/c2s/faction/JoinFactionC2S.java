...
public record JoinFactionC2S(Account account, long factionId) {
    public static void write(Output output, JoinFactionC2S o) {
        PacketUtils.writeAccount(output, o.account);
        output.writeLong(o.factionId);
    }

    public static JoinFactionC2S read(Input input) {
        return new JoinFactionC2S(PacketUtils.readAccount(input), input.readLong());
    }
}
