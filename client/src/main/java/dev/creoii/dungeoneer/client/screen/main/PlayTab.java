...
public class PlayTab extends Tab {
    ...
    protected void build() {
        ...
        raidButton.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor) {
                ...
                getClient().get().sendTCP(new JoinOrCreateRaidC2S(getClient().getState().getAccount(), getClient().getState().getActiveCharacter().get(), 4));
            }
        });
    ...
    @Override
    public void select() {
        if (getClient().getState().getActiveCharacter().isNull()) {
